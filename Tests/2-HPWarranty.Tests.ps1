Describe "HPWarranty API Operational Tests" {
    Context "Serial Number Query Test" {
        ...
        It "Serial Number MXQ84504T0 is not covered (ISEE Method)" {
            ($SNs[0] | Get-HPEntWarrantyEntitlement -QueryMethod ISEE -warningaction silentlycontinue).Covered | Should Be $false
        }
        It "Serial Number MXQ84504T0 is not covered (HPSC Method)" {
            ($SNs[0] | Get-HPEntWarrantyEntitlement -QueryMethod HPSC -warningaction silentlycontinue).Covered | Should Be $false
        }
